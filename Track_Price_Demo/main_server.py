...
if __name__ == '__main__':
    ...
    for employee, department in results:
        data.append({'Employee_name': employee.name,
                    'Department_name': department.name})
    mod_data = json.dumps(data)
    print(json.loads(mod_data))
...
'''
    ...
    db.session.add_all([dep1, dep2, dep3, dep4, dep5, dep6,
                        dep7, dep8, dep9, dep10, dep11, dep12])
    db.session.commit()
    # with open('MOCK_DATA.csv', 'r') as csv_file:

    with open(r'Track_Price_Demo\MOCK_DATA.csv', 'r') as csv_file:
        print('csv')
        csv_reader = csv.DictReader(csv_file, delimiter=',')
        for c in csv_reader:
            em = Employee(name=c['name'], salary=c['salary'],
